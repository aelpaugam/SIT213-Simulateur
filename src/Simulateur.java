...
public class Simulateur {
	...
	/** La sonde logique */
	private Sonde <Boolean> sonde = null;
	...
	/**Le message tel qu'il a été envoyé par la sonde */
	private Information <Boolean> informationEmise = null;
	/**Le message tel qu'il a été recue par la destination */
	private Information <Boolean> informationRecue = null;
	...
	public  Simulateur(String [] args) throws ArgumentsException {
		...
		//Sonde
		sonde = new SondeLogique ("Transmetteur parfait",200);
		...
		if(messageAleatoire){
			...
			else 	source = new SourceAleatoire (nbBitsMess);
	...
	public void execute() throws Exception {         
		...
		this.informationEmise = source.getInformationEmise();
		this.informationRecue = destination.getInformationRecue();
	}


	/**Méthode connection chargée de connecter les différents éléments 
	 * en vu de créer une chaîne de transmission.
	 */
	public void connection() {

		//Connection de la source au transmetteur et à la sonde.
		source.connecter(transmetteurLogique);  

		if (affichage) source.connecter(sonde);

		//Connection du transmetteur à la source et à la sonde.
		transmetteurLogique.connecter(destination);

		if (affichage) transmetteurLogique.connecter(sonde);
	...
	public float  calculTauxErreurBinaire() {
		//Initialisaion des variables
		int compteur = 0;
		int nbBitsEmis = informationEmise.nbElements();

		//Boucle de calcul du taux.
		//Un compteur est incrémenté à chaque erreur décelée.

		for (int i = 0; i<nbBitsEmis;i++){

			if (informationRecue.iemeElement(i)!=informationEmise.iemeElement(i)) compteur++ ;    		
		}
		//Le taux est obtenu en divisant le nombre de bits erronnés par le nombre total 
		//de bits envoyés.
		return  compteur/nbBitsEmis;
	}
